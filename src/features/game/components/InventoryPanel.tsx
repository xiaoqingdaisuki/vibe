...
export function InventoryPanel({ character, onAction }: InventoryPanelProps) {
  ...
  const toggleFavorite = useCallback((itemName: string, e: React.MouseEvent) => {
    e.stopPropagation()
  ...
  const canEquip = useCallback((item: Item): boolean => {
    if (item.type !== "equipment") return false
    if (!item.classRequired) return true
    return character.class === item.classRequired
  }, [character.class])

  const getItemClassLabel = useCallback((item: Item): string | null => {
          ...
          <div className={styles.pillGroup}>
            {([
              ...
              <button
                ...
                onClick={() => { setSortBy(key); handleSort() }}
              >
                {label}
              </button>
            ))}
      ...
      <div className={styles.list}>
        {aggregated.size === 0 ? (
          ...
          Array.from(aggregated.entries()).map(([type, items]) => {
            ...
            const groupRarity = items.length > 0
              ? items.reduce<ItemRarity>((best, { item }) => RARITY_INDEX[item.rarity] > RARITY_INDEX[best] ? item.rarity : best, items[0].item.rarity)
              : "common"
              ...
              <div key={type} className={styles.group}>
                ...
                >
                  ...
                  <span
                    ...
                    style={{ backgroundColor: RARITY_COLORS[groupRarity] }}
                  />
                </button>
                ...
                {!isCollapsed && (
                  <div className={styles.groupItems}>
                    {items.map(({ item, count }) => {
                      ...
                      const showDetail = item.rarity === "epic" || item.rarity === "legendary" || item.rarity === "mythic" || item.rarity === "transcendent"
                      ...
                      return (
                        ...
                        >
                          ...
                          {statPreview && (
                            <div className={styles.itemStats}>{statPreview}</div>
                          )}

                          {comparison && (
                            <div className={`${styles.itemComparison} ${comparison.startsWith("↑") ? styles.comparisonGood : styles.comparisonBad}`}>
                              {comparison}
                            </div>
                          )}
          ...
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
              ...
              return (
                <div className={styles.modalStats}>
                  {Object.entries(resolved).map(([stat, val]) => {
                    ...
                    const labels: Record<string, string> = { str: "STR", dex: "DEX", int: "INT", vit: "VIT", luk: "LUK" }
                    return (
            ...
            <div className={styles.modalActions}>
              {canEquip(detailItem) && detailItem.slot && (
                <button
                  onClick={() => { onAction({ type: "equip", slot: detailItem.slot!, itemName: detailItem.name }); setDetailItem(null) }}
                  className={`${styles.actionBtn} ${styles.equipBtn}`}
                >
                  装备
                </button>
              ...
              <button
                onClick={() => { onAction({ type: "sell", itemName: detailItem.name }); setDetailItem(null) }}
                className={`${styles.actionBtn} ${styles.sellBtn}`}
              >
                出售 +{getSellPrice(detailItem)}
              </button>
              <button
                onClick={(e) => { toggleFavorite(detailItem.name, e); setDetailItem(null) }}
