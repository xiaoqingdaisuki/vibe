...
import { copyBytesToArrayBuffer, readGbaRom } from './rom';
...
export interface GbaSessionState {
  phase: GbaPhase;
  errorMessage: string;
  ...
  speed: GbaSpeed;
  cheats: GbaCheat[];
  draftCheats: GbaCheat[];
  isCheatModalOpen: boolean;
  cheatName: string;
  cheatCode: string;
  cheatError: string;
  isApplyingCheats: boolean;
  isTouchFullscreen: boolean;
}

export interface GbaSessionActions {
  handleFileChange: (event: ChangeEvent<HTMLInputElement>) => Promise<void>;
  handleChooseRom: () => void;
  handleToggleTouchFullscreen: () => Promise<void>;
  handleTogglePause: () => void;
  handleReset: () => Promise<void>;
  handleSave: () => void;
  handleToggleMute: () => void;
  handleCycleSpeed: () => void;
  handleOpenCheats: () => void;
  handleCloseCheats: () => void;
  handleCheatBackdropClick: (event: ReactMouseEvent<HTMLDivElement>) => void;
  handleCheatNameChange: (value: string) => void;
  handleCheatCodeChange: (value: string) => void;
  handleAddCheat: () => void;
  handleUpdateCheat: (id: string, patch: Partial<Omit<GbaCheat, 'id'>>) => void;
  handleRemoveCheat: (id: string) => void;
  handleApplyCheats: () => Promise<void>;
  handleExportSave: () => void;
  handleButtonPress: (button: GbaButton) => void;
  handleButtonRelease: (button: GbaButton) => void;
}
...
export interface GbaSession {
  refs: GbaSessionRefs;
  state: GbaSessionState;
  actions: GbaSessionActions;
}

// 管理 GBA 的 WASM、存档、输入和金手指会话，页面只负责组合展示
export function useGbaSession(): GbaSession {
  // 保存应用根节点，作为移动端全屏横屏容器
  ...
  function isCurrentLoad(requestId: number): boolean {
    return mountedRef.current && loadRequestRef.current === requestId;
  }
  ...
  async function persistCurrentSave(): Promise<void> {
    ...
    try {
      ...
      if (emulatorRef.current !== emulator || romRef.current !== currentRom) return;
      ...
      if (mountedRef.current && emulatorRef.current === emulator && romRef.current === currentRom) {
  ...
  async function createLoadedEmulator(
    ...
    const nextEmulator = await createGbaEmulator(canvasRef.current, {
      ...
      onSaveDirty: () => {
        if (isCurrentLoad(requestId)) saveDirtyRef.current = true;
      },
    });
  ...
  async function stopCurrentEmulator(requestId?: number): Promise<boolean> {
    await persistCurrentSave();
    if (requestId !== undefined && !isCurrentLoad(requestId)) return false;
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>): Promise<void> {
    ...
    try {
      ...
      if (!isCurrentLoad(requestId)) return;
      if (!(await stopCurrentEmulator(requestId))) return;
      setRom(nextRom);
      ...
      loadingEmulator = await createLoadedEmulator(nextRom, storedSave?.data ?? null, storedCheats, requestId);
      if (!isCurrentLoad(requestId)) {
  ...
  // 更新新增金手指的名称输入
  function handleCheatNameChange(value: string): void {
    setCheatName(value);
  }

  // 更新新增金手指的代码输入
  function handleCheatCodeChange(value: string): void {
    setCheatCode(value);
  }
