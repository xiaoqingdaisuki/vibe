...
export class GameEngine {
  ...
  runCombat(monsterLevel: number): CombatResult {
    ...
    while (playerHp > 0 && monsterHp > 0 && rounds < 100) {
      ...
      if (learnedSkills.length > 0) {
        const skillResult = this.tryActivateSkill(
          learnedSkills,
          appliedBuffs,
          skillCooldowns,
          playerHp,
        ...
        if (skillResult) {
          ...
          if (category === 'damage') {
            ...
            const damageSource = this.character.class === 'mage' ? this.getSpellPower() : playerAtk;
      ...
      if (dodgeRoll) {
        ...
        const reduction =
          this.getActiveBuffValue(appliedBuffs, learnedSkills, 'shield_wall') +
          this.getActiveBuffValue(appliedBuffs, learnedSkills, 'magic_shield');
    ...
    for (let i = 0; i < combats; i++) {
      ...
      for (let retry = 0; retry < 30; retry++) {
        ...
        if (victory) {
          ...
          result.totalGoldGained += Math.floor(
            (monster.goldReward + Math.floor(Math.random() * monsterLevel * 3)) * diffMult,
          );
          break;
        }
  ...
  formatCombatDetail(result: CombatResult): string {
    ...
    lines.push(
      `结果：${result.victory ? '胜利' : '失败'} | 总伤害：${result.playerDmgDealt} | 总承伤：${result.playerDmgTaken}`,
    );
    return lines.join('\n');
  }
  // 根据等级获取难度 tier
  ...
  private openChest(itemId: string): LogEntry[] {
    ...
    const addSkillBook = () => {
      ...
      const book =
        classBooks.length > 0
          ? classBooks[Math.floor(Math.random() * classBooks.length)]
          : ITEMS.find((it) => it.id === 'skill_book_warrior');
  ...
  private useSkillBook(item: Item): LogEntry[] {
    ...
    const learnedClassSkills = this.character.skills.filter((sk) => availableSkills.some((s) => s.id === sk.skillId));
